...
export interface Session { id: string; project: string; parent: string | null; title: string; closed: boolean }
...
export interface Bootstrap { workspace: Workspace; pendingOperations?:import('./operations.svelte').PendingOperation[]; preferences: Preferences | null; config: string; configError: string | null; configPath: string; token: string }
...
export interface Run { id: string; workspace: string; phase: string; busy: boolean; task: string; status: string; diff: string; gates: string; review: string; summary: string; deliverable: {root: string; paths: string[]; instructions: string; qualification: string} | null }
export interface Conversation { run?: Run; received?: string; incorporated?: string; activities?: Activity[] }
