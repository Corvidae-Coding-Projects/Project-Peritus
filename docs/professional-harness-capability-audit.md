...
| Rate limits, queue backpressure, provider availability, and failover | Working with a boundary | Direct providers classify rate limits and use bounded retry policy; D3 and application protocols model backpressure. G4 offers durable, default-off automatic-failover consent when at least two providers are configured. After ordinary recovery ends, each designer, writer, reviewer, or fixer role may advance through deterministic tool-compatible providers. A failed profile's circuit remains open across later roles in the same run when a healthy consented fallback exists, avoiding repeated calls to a route already proven unavailable. Capability mismatch, authentication, quota, rate, timeout, malformed, and safe provider failures are eligible; safety, refusal, cancellation, raw ambiguous transport, and normalized ambiguous acceptance are not. Every switch is durably traced and counted in persisted live status. The serialized Terminal-Bench adapter now preserves official Claude CLI OAuth rotation across disposable containers through a bounded, non-rollback, compare-before-replace checkpoint (`TBF-032`, `TBF-057`). | Preserve the explicit consent, run-scoped circuit, and terminal taxonomy. A predictive fleet-health queue remains out of scope until real multi-user or high-concurrency evidence requires it. |
...
| Bounded tool output and long-running command control | Working | Structured commands cap both streams, close stdin, own the process tree, accept 1-600 second requested deadlines, and kill/reap on expiry. Each actual allowance also shrinks against the live caller-derived product horizon while preserving a bounded completion reserve; the result exposes both timeouts and the remaining budget. A capped stream retains its opening context and final diagnostics while omitting the noisy middle. Roles must filter predictable bulk output before execution and select decisive fields, keys, counts, or bounded samples from structured and API responses. Binary, deleted, damaged, or truncated data starts from contract-supplied stable fragments and bounded neighboring windows before speculative transforms. Workspace growth and observed harness memory now join the cumulative run projection. Terminal-Bench `TBF-003` reproduced the freeze, `TBF-020` exposed low-signal context consumption, `TBF-023` showed why a long dependency build must retain its terminal error, `TBF-044` showed why individually bounded commands must share the enclosing deadline, and `TBF-053` distinguishes a useful partial-record search from both an over-constrained whole-record filter and an unbounded dump. | Preserve and compare the observations with host-level soak evidence. |
| Interactive subprocess control | Working | The ordinary G4 developer loop now routes both finite commands and active commands through C4's router and the daemon-owned C2 process store. `run_command` remains the simple synchronous path. `command_start` returns a stable run-owned handle, and `command_poll`, `command_stdin`, `command_resize`, `command_signal`, `command_cancel`, and `command_recover` expose bounded PTY or pipe control without adding another process manager. Exact C4 and C2 authority is committed for every start, output remains artifact-backed and bounded, terminal observations are stable, and a router-deadline race now collects an already-terminal C2 result instead of reporting a false ownership failure. Focused tests cover interactive stdin, cancellation, timeout, missing executables, bounded stdout/stderr, resource clamping, and command-created files. Terminal-Bench `TBF-047` supplied the general failure evidence. | Preserve the single C4/C2 ownership path. Recovery reconciles a handle retained by the live run; daemon restart still uses C2 startup reconciliation and must not claim that an arbitrary lost PTY session can be reconstructed. |
...
6. Completed on the draft qualification branch: default-off compatible role failover after
   same-provider recovery, run-scoped provider circuits shared across roles, durable switch
   evidence, persisted counters, and an explicit exclusion for safety, refusal, cancellation, and
   ambiguous acceptance. Predictive queue health remains evidence-gated rather than speculative.
7. Completed on the draft qualification branch: route the G4 product developer loop through C4's
   existing active shell execution and the daemon-owned C2 process store. Models can start, poll,
   write bounded stdin, resize, signal, cancel, and recover live PTY or pipe processes by stable
   run-owned handle without bypassing C2/C3 ownership or introducing a second process manager.
