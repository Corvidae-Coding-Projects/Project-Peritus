...
| Crash-safe replay and daemon restart | Working with a boundary | Product snapshots and conversations are atomically persisted; unfinished runs reopen as `RecoveryRequired` and can be retried against the managed worktree. Exact in-flight provider state and the newly captured developer-command review ledger are not resumed byte-for-byte. | Persist the compacted context and effect receipts needed for exact safe continuation; do not claim exact provider-stream resumption where a provider lacks it. |
| Context-window accounting | Partial | C6 has verified token budgets, context selection, provenance, render plans, and compaction validation. The G4 developer loop does not use them; it replays its entire role/tool transcript until a protocol or provider limit stops it. | Add a product context controller before the full final benchmark rerun. |
| Deterministic context compaction | Missing from G4 | C6 validates proposed compactions, but no product-loop trigger, compacted developer transcript, durable summary, or invalidation path is wired into `DeveloperLoop`. | Compact older completed tool exchanges while preserving system policy, the current task, unresolved findings, recent turns, and exact source digests. Record why and when compaction occurred. |
| Compaction provenance, freshness, and invalidation | Missing from G4 | The C6 substrate can bind source ranges and digests, but product-run persistence stores only the human conversation and finding state. | Persist a versioned compaction record bound to source message/tool digests, policy revision, provider profile, and conversation revision. Rebuild when any source changes. |
| Provider-aware prompt caching | Partial | C5 profiles, request encoding, and usage counters support prompt caching. The D0 developer loop always requests `CachePolicy::Disabled`. Account-backed Codex still reported substantial implicit cached input during qualification, but that is not a Peritus-controlled policy. | Negotiate `PromptCaching`, use `Automatic` only when supported, keep stable prefixes, and report cache reads/creation separately. |
...
1. Finish the Terminal-Bench baseline and classify every failure without changing the running
   binary.
2. Add a G4 context controller: deterministic input accounting, durable compaction records,
   preservation rules, freshness checks, and trace observations.
3. Negotiate prompt caching and retain cache accounting while preserving local-first provider
   storage policy.
