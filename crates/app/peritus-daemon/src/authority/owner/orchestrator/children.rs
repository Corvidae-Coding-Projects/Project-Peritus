//! Durable D1/D2 pause and resume admission before E0/C0 claim settlement.
...
pub(super) fn deliver_child_directive(
    ...
    let head = checked_child_head(&orchestrator, claim)?;
    match claim.directive().destination() {
        ...
        DirectiveDestination::Review => {
            admit_review_lifecycle(journal, &orchestrator, claim, head)?;
        }
        _ => return Err(child_mismatch("child lifecycle handler received another destination")),
    }
    settle_claimed_directive(journal, claim)
}

fn checked_child_head(
    ...
    if !matches!(directive.kind(), DirectiveKind::PauseChildren | DirectiveKind::ResumeChildren) {
        return Err(child_mismatch("E0 child directive is not pause or resume"));
    }
    ...
    let expected = directive_payload_digest(
        directive.kind(),
    ...
    if directive.payload_digest() != expected {
    ...
    let aggregate = match directive.destination() {
        ...
        _ => return Err(child_mismatch("child lifecycle destination is unsupported")),
    };
    reconciliation
        .child_heads()
        .iter()
...
pub(super) fn commit_review_lifecycle(
    journal: &mut SqliteJournal,
...
fn exact_review_retry(
    ...
    let exact_identity =
        [last.id() == command.event_id(), last.command_id() == command.command_id()]
            .into_iter()
...
fn child_mismatch(detail: &'static str) -> DaemonError {
    DaemonError::new(
        ...
        "deliver E0 child lifecycle directive",
        detail,
    )
}
