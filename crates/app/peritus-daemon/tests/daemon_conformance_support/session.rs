...
pub(super) fn new_command() -> io::Result<DaemonConformanceObservation> {
    ...
    let events = result.committed_events().map_or(0, |range| range.count());
...
pub(super) fn replay_command() -> io::Result<DaemonConformanceObservation> {
    ...
    Ok(command_observation(
        ...
        replayed.committed_events().map_or(0, |range| range.count()),
        exact,
        exact,
        0,
        0,
...
pub(super) fn indeterminate_command() -> io::Result<DaemonConformanceObservation> {
    let environment = TestEnvironment::new()?;
    ...
    let seeded = NewApplicationCommand::new(
        ActorId::new([0x22; 16]).map_err(super::debug_error)?,
        session,
        binding.idempotency_key().as_bytes().to_vec(),
        binding.request_digest().as_sha256(),
        binding.frames().command_frame().digest(),
        ApplicationRequestId::new(binding.request_id().into_bytes()).map_err(super::debug_error)?,
        command_id,
    )
    .map_err(super::debug_error)?;
    let ApplicationCommandAdmission::Inserted(record) =
        journal.admit_application_command(seeded).map_err(super::debug_error)?
    else {
    ...
    let command_count = u64::try_from(
        connection
            .query_row("SELECT COUNT(*) FROM app_commands", [], |row| row.get::<_, i64>(0))
            .map_err(super::debug_error)?,
    )
    .map_err(super::debug_error)?;
    let event_count = u64::try_from(
        connection
