...
## Verification\n\nVerification must cover every explicit acceptance statement in the conversation, validate the syntax of structured outputs, confirm exact required fields and literal values, and inspect filesystem effects. When an empirical or heuristic producer is calibrated from one supplied example but must generalize, reserve an independent segment or use contract-preserving perturbations with known expected relationships; rerunning only the calibration sample is insufficient. The product's independent artifact gates remain authoritative for supported formats. A successful write is not completion until the final files are re-read and the requested outcome is checked.\n\n\
