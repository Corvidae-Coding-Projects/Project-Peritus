//! Exact request invocation identity that owns an immutable input binding.
...
impl TryFrom<[u8; 16]> for InvocationId {
...
impl std::fmt::Debug for InvocationId {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "InvocationId({self})")
    }
}
