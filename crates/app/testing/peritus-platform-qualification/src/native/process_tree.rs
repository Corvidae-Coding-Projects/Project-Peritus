//! Cross-platform ownership of one native controller process tree.
    ...
    impl ProcessTree {
        ...
        pub fn attach(child: &Child, _maximum_processes: u32) -> Result<Self, QualificationError> {
            let group = i32::try_from(child.id()).map_err(|_| {
                native_error("own native H2 process tree", "child PID exceeds Unix process range")
            })?;
            Ok(Self { group })
        }
    ...
    fn signal_group(group: i32, signal: Signal) -> Result<(), QualificationError> {
        match kill(Pid::from_raw(-group), Some(signal)) {
            ...
            Err(error) => Err(native_error(
                "terminate native H2 process tree",
...
mod platform {
    ...
    impl ProcessTree {
        ...
        pub fn attach(child: &Child, maximum_processes: u32) -> Result<Self, QualificationError> {
            ...
            // SAFETY: the job and correctly sized immutable limits remain live for this call.
            ...
            // SAFETY: the std child owns a live process handle and `tree` owns the live job.
    ...
    fn job_error(action: &str) -> QualificationError {
        native_error("own native H2 process tree", format!("{action} failed"))
    }
}
