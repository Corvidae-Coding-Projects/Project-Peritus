...
fn v1_journal_rows_migrate_byte_exactly_and_new_aggregate_records_append() {
    ...
    let mut engine =
        MigrationEngine::open(config(&temp, database.clone()), MigrationRegistry::current())
            .expect("migration engine");
    let plan = engine.preflight(version(10)).expect("v10 preflight").into_plan();
    assert_eq!(plan.current_version(), 1);
    assert!(plan.backup_required());
    let applied = engine.apply(&plan, operation(10)).expect("v10 apply");
    assert!(applied.backup_path().expect("required backup").is_file());
    drop(engine);
    ...
    append_new_aggregate(&mut journal, AggregateKind::Harness, 90, 91, 92, 80);
    assert_eq!(journal.integrity_scan().expect("post-upgrade integrity").event_count(), 9);
}
...
fn v3_fixture_preserves_every_historical_aggregate_tag_through_d2_and_d3_migrations() {
    ...
    assert!(plan.backup_required());
    let applied = engine.apply(&plan, operation(11)).expect("v10 apply");
    assert!(applied.backup_path().expect("required backup").is_file());
    drop(engine);
    ...
    append_new_aggregate(&mut journal, AggregateKind::Harness, 130, 131, 132, 80);
    assert_eq!(journal.integrity_scan().expect("new aggregate integrity").event_count(), 13);
    drop(journal);
...
fn v4_fixture_preserves_tags_one_through_nine_and_admits_d3_e0_aggregates() {
    ...
    assert!(plan.backup_required());
    let applied = engine.apply(&plan, operation(12)).expect("v10 apply");
    assert!(applied.backup_path().expect("required backup").is_file());
    drop(engine);
    ...
    append_new_aggregate(&mut journal, AggregateKind::Harness, 160, 161, 162, 80);
    assert_eq!(journal.integrity_scan().expect("D3/E0/E1 integrity").event_count(), 13);
    drop(journal);
...
#[test]
fn v5_fixture_preserves_tags_one_through_twelve_and_admits_e1_harness() {
    let temp = tempfile::tempdir().expect("tempdir");
    let database = temp.path().join("journal-v5.sqlite3");
    let connection = rusqlite::Connection::open(&database).expect("v5 connection");
    let fixture = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/v5.sql");
    connection
        .execute_batch(&fs::read_to_string(fixture).expect("read v5 fixture"))
        .expect("install frozen v5 schema");
    for tag in 1_u8..=12 {
        insert_v3_record(&connection, tag, tag.saturating_mul(10), 40 + u16::from(tag));
    }
    let preserved = snapshot_v3_rows(&connection);
    ...
    let mut engine =
        MigrationEngine::open(config(&temp, database.clone()), MigrationRegistry::current())
            .expect("migration engine");
    let plan = engine.preflight(version(10)).expect("v10 preflight").into_plan();
    assert_eq!(plan.current_version(), 5);
    assert!(plan.backup_required());
    let applied = engine.apply(&plan, operation(13)).expect("v10 apply");
    assert!(applied.backup_path().expect("required backup").is_file());
    drop(engine);
    ...
    .expect("schema-v10 journal");
    assert_eq!(journal.integrity_scan().expect("migrated integrity").event_count(), 12);
    append_new_aggregate(&mut journal, AggregateKind::Harness, 200, 201, 202, 80);
    assert_eq!(journal.integrity_scan().expect("E1 integrity").event_count(), 13);
    drop(journal);

    let mut rollback =
        MigrationEngine::open(config(&temp, database.clone()), MigrationRegistry::current())
            .expect("rollback engine");
    let restored = rollback.restore_backup(operation(13)).expect("restore v5 backup");
    assert_eq!(restored.state(), RecoveryState::Restored);
    drop(rollback);
    let restored = rusqlite::Connection::open(database).expect("restored v5 fixture");
    assert_eq!(snapshot_v3_rows(&restored), preserved);
    assert_eq!(
        restored
            .pragma_query_value(None, "user_version", |row| row.get::<_, i64>(0))
...
#[test]
fn v6_fixture_preserves_tags_one_through_thirteen_and_admits_e2_debugger() {
    let temp = tempfile::tempdir().expect("tempdir");
    let database = temp.path().join("journal-v6.sqlite3");
    let connection = rusqlite::Connection::open(&database).expect("v6 connection");
    let fixture = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/v6.sql");
    connection
        .execute_batch(&fs::read_to_string(fixture).expect("read v6 fixture"))
        .expect("install frozen v6 schema");
    for tag in 1_u8..=13 {
        insert_v3_record(&connection, tag, tag.saturating_mul(10), 40 + u16::from(tag));
    }
    let preserved = snapshot_v3_rows(&connection);
    ...
    let mut engine =
        MigrationEngine::open(config(&temp, database.clone()), MigrationRegistry::current())
            .expect("migration engine");
    let plan = engine.preflight(version(10)).expect("v10 preflight").into_plan();
    assert_eq!(plan.current_version(), 6);
    assert!(plan.backup_required());
    let applied = engine.apply(&plan, operation(14)).expect("v10 apply");
    assert!(applied.backup_path().expect("required backup").is_file());
    drop(engine);
    ...
    .expect("schema-v10 journal");
    assert_eq!(journal.integrity_scan().expect("migrated integrity").event_count(), 13);
    append_new_aggregate(&mut journal, AggregateKind::Debugger, 220, 221, 222, 83);
    assert_eq!(journal.integrity_scan().expect("E2 integrity").event_count(), 14);
    drop(journal);

    let mut rollback =
        MigrationEngine::open(config(&temp, database.clone()), MigrationRegistry::current())
            .expect("rollback engine");
    let restored = rollback.restore_backup(operation(14)).expect("restore v6 backup");
    assert_eq!(restored.state(), RecoveryState::Restored);
    drop(rollback);
    let restored = rusqlite::Connection::open(database).expect("restored v6 fixture");
    assert_eq!(snapshot_v3_rows(&restored), preserved);
    assert_eq!(
        restored
            .pragma_query_value(None, "user_version", |row| row.get::<_, i64>(0))
...
#[test]
fn v7_fixture_preserves_tags_one_through_fourteen_and_admits_e3_evaluation() {
    let temp = tempfile::tempdir().expect("tempdir");
    let database = temp.path().join("journal-v7.sqlite3");
    let connection = rusqlite::Connection::open(&database).expect("v7 connection");
    let fixture = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/v7.sql");
    connection
        .execute_batch(&fs::read_to_string(fixture).expect("read v7 fixture"))
        .expect("install frozen v7 schema");
    for tag in 1_u8..=14 {
        insert_v3_record(&connection, tag, tag.saturating_mul(10), 40 + u16::from(tag));
    }
    let preserved = snapshot_v3_rows(&connection);
    ...
    let mut engine =
        MigrationEngine::open(config(&temp, database.clone()), MigrationRegistry::current())
            .expect("migration engine");
    let plan = engine.preflight(version(10)).expect("v10 preflight").into_plan();
    assert_eq!(plan.current_version(), 7);
    assert!(plan.backup_required());
    let applied = engine.apply(&plan, operation(15)).expect("v10 apply");
    assert!(applied.backup_path().expect("required backup").is_file());
    drop(engine);
    ...
    .expect("schema-v10 journal");
    assert_eq!(journal.integrity_scan().expect("migrated integrity").event_count(), 14);
    append_new_aggregate(&mut journal, AggregateKind::Evaluation, 230, 231, 232, 85);
    assert_eq!(journal.integrity_scan().expect("E3 integrity").event_count(), 15);
    drop(journal);

    let mut rollback =
        MigrationEngine::open(config(&temp, database.clone()), MigrationRegistry::current())
            .expect("rollback engine");
    let restored = rollback.restore_backup(operation(15)).expect("restore v7 backup");
    assert_eq!(restored.state(), RecoveryState::Restored);
    drop(rollback);
    let restored = rusqlite::Connection::open(database).expect("restored v7 fixture");
    assert_eq!(snapshot_v3_rows(&restored), preserved);
    assert_eq!(
        restored
            .pragma_query_value(None, "user_version", |row| row.get::<_, i64>(0))
...
#[test]
fn v8_fixture_preserves_tags_one_through_fifteen_and_admits_f0_aggregates() {
    let temp = tempfile::tempdir().expect("tempdir");
    let database = temp.path().join("journal-v8.sqlite3");
    let connection = rusqlite::Connection::open(&database).expect("v8 connection");
    let fixture = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/v8.sql");
    connection
        .execute_batch(&fs::read_to_string(fixture).expect("read v8 fixture"))
        .expect("install frozen v8 schema");
    for tag in 1_u8..=15 {
        insert_v3_record(&connection, tag, tag.saturating_mul(10), 40 + u16::from(tag));
    }
    let preserved = snapshot_v3_rows(&connection);
    ...
    assert!(plan.backup_required());
    let applied = engine.apply(&plan, operation(16)).expect("v10 apply");
    assert!(applied.backup_path().expect("required backup").is_file());
    drop(engine);
    ...
    .expect("schema-v10 journal");
    assert_eq!(journal.integrity_scan().expect("migrated integrity").event_count(), 15);
    append_new_aggregate(&mut journal, AggregateKind::EvolutionCampaign, 240, 241, 242, 89);
    append_new_aggregate(&mut journal, AggregateKind::ProductionHarness, 243, 244, 245, 92);
    assert_eq!(journal.integrity_scan().expect("F0 integrity").event_count(), 17);
    drop(journal);

    let mut rollback =
        MigrationEngine::open(config(&temp, database.clone()), MigrationRegistry::current())
            .expect("rollback engine");
    let restored = rollback.restore_backup(operation(16)).expect("restore v8 backup");
    assert_eq!(restored.state(), RecoveryState::Restored);
    drop(rollback);
    let restored = rusqlite::Connection::open(database).expect("restored v8 fixture");
    assert_eq!(snapshot_v3_rows(&restored), preserved);
    assert_eq!(
        restored
            .pragma_query_value(None, "user_version", |row| row.get::<_, i64>(0))
...
#[test]
fn v9_fixture_installs_exact_application_schema_and_restores_without_it() {
    let temp = tempfile::tempdir().expect("tempdir");
    let database = temp.path().join("journal-v9.sqlite3");
    let connection = rusqlite::Connection::open(&database).expect("v9 connection");
    let fixture = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/v9.sql");
    connection
        .execute_batch(&fs::read_to_string(fixture).expect("read v9 fixture"))
        .expect("install frozen v9 schema");
    for tag in 1_u8..=17 {
        insert_v3_record(&connection, tag, tag.saturating_mul(10), 40 + u16::from(tag));
    }
    let preserved = snapshot_v3_rows(&connection);
    assert!(application_schema(&connection).is_empty());
    drop(connection);
    ...
    let mut engine =
        MigrationEngine::open(config(&temp, database.clone()), MigrationRegistry::current())
            .expect("migration engine");
    let plan = engine.preflight(version(10)).expect("v10 preflight").into_plan();
    assert_eq!(plan.current_version(), 9);
    assert!(plan.backup_required());
    let applied = engine.apply(&plan, operation(17)).expect("v10 apply");
    assert!(applied.backup_path().expect("required backup").is_file());
    drop(engine);
    ...
    let installed_database = temp.path().join("installed-v10.sqlite3");
    let installed_journal = SqliteJournal::open(
        &installed_database,
        StoreId::new([2; 16]).expect("installed store identity"),
        SqliteJournalOptions::default(),
    )
    .expect("install current journal schema");
    drop(installed_journal);
    let installed = rusqlite::Connection::open(installed_database).expect("installed connection");
    assert_eq!(application_schema(&connection), application_schema(&installed));
    drop(installed);
    drop(connection);
    ...
    .expect("schema-v10 journal");
    assert_eq!(journal.integrity_scan().expect("migrated integrity").event_count(), 17);
    drop(journal);

    let mut rollback =
        MigrationEngine::open(config(&temp, database.clone()), MigrationRegistry::current())
            .expect("rollback engine");
    let restored = rollback.restore_backup(operation(17)).expect("restore v9 backup");
    assert_eq!(restored.state(), RecoveryState::Restored);
    drop(rollback);
    let restored = rusqlite::Connection::open(database).expect("restored v9 fixture");
    assert_eq!(snapshot_v3_rows(&restored), preserved);
    assert_eq!(
        restored
            .pragma_query_value(None, "user_version", |row| row.get::<_, i64>(0))
    ...
    assert!(application_schema(&restored).is_empty());
}

fn application_schema(connection: &rusqlite::Connection) -> Vec<(String, String, String)> {
    ...
    statement
        .query_map([], |row| {
            let sql = row.get::<_, String>(2)?.replace(" IF NOT EXISTS", "");
            Ok((row.get(0)?, row.get(1)?, sql))
        })
        .expect("application schema rows")
        .map(|row| row.expect("application schema row"))
        .collect()
}
