...
pub fn profile(
    ...
    ProviderProfile::new(
        ...
        ProviderName::new("openai".to_owned())
            .map_err(|_| ProviderConformanceError::Infrastructure)?,
        ModelName::new(format!("codex-runtime-{}", super::slug(scenario)))
            .map_err(|_| ProviderConformanceError::Infrastructure)?,
...
pub fn request(
    ...
    let tools = if with_tools { vec![tool(canary)?] } else { Vec::new() };
    ...
    ModelRequest::new(
        profile,
        negotiated,
        RequestId::new(canary.unwrap_or("codex-runtime-conformance").to_owned())
            .map_err(|_| ProviderConformanceError::Infrastructure)?,
        vec![system, user],
        tools,
        tool_choice,
