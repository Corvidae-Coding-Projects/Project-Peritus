//! Canary injection accounting and normalized/reportable-surface scanning.
...
use peritus_model_protocol::{CachePolicy, ContentBlock, EventEnvelope, ModelEvent, ModelRequest};
...
pub(super) fn observe(
    ...
    surfaces.push(
        ReportText::new(
            "OpenAI fixture manifest, digest inventory, artifacts, and normalized fields contain no canary",
        )
...
pub(super) fn request_canary_count(request: &ModelRequest, canary: &str) -> usize {
    ...
    let prompts = request
        ...
        .count();
    let cache = usize::from(matches!(
        request.options().cache(),
        CachePolicy::Explicit(key) if key.expose_for_wire() == canary
    ));
    request_id + prompts + cache
}
...
fn fixture_artifacts_contain(canary: &str) -> Result<bool, ProviderConformanceError> {
    for inventory in [fixture("MANIFEST"), fixture("SHA256SUMS")] {
        if contains_bytes(&inventory, canary.as_bytes()) {
            return Ok(true);
        }
    }
    ...
    for line in manifest.lines().skip_while(|line| !line.starts_with("reviewed=")).skip(1) {
        ...
        if contains_bytes(&fixture(name), canary.as_bytes()) {
            return Ok(true);
        }
    }
    Ok(false)
