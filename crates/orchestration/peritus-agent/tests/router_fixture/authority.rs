//! B1 capability/budget transitions and C0 committed observations.
...
pub fn commit_authority(
    ...
    let action_digest = intent.digest(CodecLimits::PRODUCTION).unwrap();
    ...
    journal_store.allocate_authority_epoch(ExpectedAuthorityEpoch::Absent).unwrap();
    let epoch = journal_store.current_authority_epoch().unwrap().unwrap();
...
fn capability_use(ids: &Ids, action_digest: Sha256Digest) -> CapabilityUseTransition {
    let validity = ValidityWindow::new(instant(10), instant(100)).unwrap();
    let use_limit = UseLimit::limited(3).unwrap();
    let permissions = PermissionSet::new(vec![permission(ids)]).unwrap();
    ...
    let boundary = AuthorityBoundary::new(
        ...
        vec![ids.environment],
        PermissionSet::new(vec![permission(ids)]).unwrap(),
        ids.revision,
        validity,
        use_limit,
    )
...
fn commit_budget(
    ...
    let transition = ledger.transition(BudgetCommand::Begin(request)).unwrap();
