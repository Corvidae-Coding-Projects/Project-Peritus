...
use crate::{
    ErrorCode, ReadOnlyOpenRequest, RecoveryClass, SnapshotIdentity, WorkspaceError,
    WorkspaceOperation,
};
...
impl ReadOnlyWorkspace {
    ...
    pub fn open(request: ReadOnlyOpenRequest) -> Result<Self, WorkspaceError> {
        let (repository, worktree, snapshot, writer_root) = request.into_parts();
        ...
        Ok(Self { repository, worktree, snapshot })
    }
