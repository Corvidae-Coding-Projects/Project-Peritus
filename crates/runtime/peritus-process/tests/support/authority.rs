...
use peritus_journal::{
    ...
    CurrentAuthorityEpoch, ExpectedAuthorityEpoch, HeadExpectation, SqliteJournal,
};
use peritus_policy::{
...
use peritus_process::{EXECUTION_INTENT_MEDIA_TYPE, ExecutionIntentPayload, ExecutionPlan};
...
pub struct WritableAuthorityReceipts {
    pub kernel: CommittedKernelTransition,
    pub capability: CommittedCapabilityUse,
    pub budget: CommittedBudgetTransition,
...
pub fn intent(ids: &Ids, plan: &ExecutionPlan) -> ActionIntentDto {
    ActionIntentDto {
...
pub fn commit_authority(
    ...
    commit_authority_inner(journal, ids, intent, wall_millis, true)
}

pub fn commit_authority_without_dispatch(
    journal: &mut SqliteJournal,
    ids: &Ids,
    intent: &ActionIntentDto,
    wall_millis: u64,
) -> AuthorityReceipts {
    commit_authority_inner(journal, ids, intent, wall_millis, false)
}

pub fn commit_authority_with_lease(
    journal: &mut SqliteJournal,
    ids: &Ids,
    intent: &ActionIntentDto,
    wall_millis: u64,
    ...
    WritableAuthorityReceipts { kernel, capability, budget, lease, epoch, observed_at: instant(20) }
}

fn commit_authority_inner(
    journal: &mut SqliteJournal,
    ids: &Ids,
    intent: &ActionIntentDto,
    wall_millis: u64,
...
fn commit_budget(
    ...
    let ledger = BudgetLedger::new_root(ids.execution_budget, ids.revision, limits);
    let request = BudgetRequest::new(
        ids.reservation,
        ids.execution_budget,
        ids.revision,
        ids.action,
        action_digest,
