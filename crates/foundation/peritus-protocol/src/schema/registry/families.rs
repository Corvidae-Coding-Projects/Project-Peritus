//! Complete stable message-family allocation.

use super::{ARTIFACT_SCOPE_CLAIM_FAMILY, ARTIFACT_UPLOAD_ACCEPTED_FAMILY, MessageFamily};

/// Complete B3 family registry, strictly ordered by tag.
pub const FAMILIES: &[MessageFamily] = &[
    MessageFamily {
        tag: 1,
        name: "kernel-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: false,
    },
    MessageFamily {
        tag: 2,
        name: "command-envelope",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: false,
    },
    MessageFamily {
        tag: 3,
        name: "kernel-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 4,
        name: "kernel-error",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 5,
        name: "lifecycle-phase",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 10,
        name: "budget-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: false,
    },
    MessageFamily {
        tag: 11,
        name: "budget-amounts",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: false,
    },
    MessageFamily {
        tag: 12,
        name: "budget-snapshot",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 13,
        name: "reservation-snapshot",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 14,
        name: "budget-receipt",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 15,
        name: "budget-error",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 20,
        name: "action-intent",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: false,
    },
    MessageFamily {
        tag: 21,
        name: "policy-definition",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: false,
    },
    MessageFamily {
        tag: 22,
        name: "policy-amendment-content",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 23,
        name: "policy-amendment",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: false,
    },
    MessageFamily {
        tag: 30,
        name: "acceptance-contract-content",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 31,
        name: "acceptance-contract",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: false,
    },
    MessageFamily {
        tag: 40,
        name: "agent-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 41,
        name: "agent-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 42,
        name: "agent-state",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 50,
        name: "gate-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 51,
        name: "gate-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 52,
        name: "gate-state",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 53,
        name: "review-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 54,
        name: "review-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 55,
        name: "review-state",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 60,
        name: "trace-observation",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 70,
        name: "scheduler-command",
        schema_version: 2,
        supported_schema_versions: &[1, 2],
        inert_only: true,
    },
    MessageFamily {
        tag: 71,
        name: "scheduler-event",
        schema_version: 2,
        supported_schema_versions: &[1, 2],
        inert_only: true,
    },
    MessageFamily {
        tag: 72,
        name: "scheduler-state",
        schema_version: 2,
        supported_schema_versions: &[1, 2],
        inert_only: true,
    },
    MessageFamily {
        tag: 73,
        name: "collaboration-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 74,
        name: "collaboration-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 75,
        name: "collaboration-state",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 76,
        name: "orchestrator-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 77,
        name: "orchestrator-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 78,
        name: "orchestrator-state",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 79,
        name: "harness-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 80,
        name: "harness-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 81,
        name: "harness-state",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 82,
        name: "debugger-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 83,
        name: "debugger-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 84,
        name: "debugger-state",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 85,
        name: "evaluation-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 86,
        name: "evaluation-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 87,
        name: "evaluation-state",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 88,
        name: "evolution-campaign-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 89,
        name: "evolution-campaign-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 90,
        name: "evolution-campaign-state",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 91,
        name: "production-harness-command",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 92,
        name: "production-harness-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 93,
        name: "production-harness-state",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: 94,
        name: "credential-registry-event",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: ARTIFACT_SCOPE_CLAIM_FAMILY,
        name: "application-artifact-scope-claim",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
    MessageFamily {
        tag: ARTIFACT_UPLOAD_ACCEPTED_FAMILY,
        name: "application-artifact-upload-accepted",
        schema_version: 1,
        supported_schema_versions: &[1],
        inert_only: true,
    },
];
