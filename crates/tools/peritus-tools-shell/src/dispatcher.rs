...
///
/// This route preserves C2 process ownership and C4 active-execution semantics when the product
/// boundary has explicitly authorized ordinary host execution but no native restricted C3 backend
/// is available. It cannot accept a restricted plan and is not a sandbox fallback.
pub struct RawShellDispatcher<'gateway, 'authority> {
    gateway: &'gateway ExecutionGateway,
    authorization: &'gateway ExecutionAuthorizationRequest<'authority>,
    plan: Option<ExecutionPlan>,
    artifacts: Option<ArtifactStore>,
...
impl<'gateway, 'authority> RawShellDispatcher<'gateway, 'authority> {
    ...
    ///
    /// # Errors
    /// Returns a typed failure unless the plan is explicitly raw and its caller binding names and
    ...
    pub fn new(
        ...
        let descriptor = bound_descriptor(&plan)?;
        if plan.isolation() != ExecutionIsolation::ExplicitRawEffect {
            return Err(mismatch("raw shell dispatcher requires an explicit raw-effect plan"));
        }
        Ok(Self {
            gateway,
            authorization,
    ...
    B: NativeSandboxBackend,
{
    ...
    fn start(&mut self, invocation: AuthorizedInvocation) -> Result<ToolStart, DispatchFailure> {
        ...
        validate_invocation(&self.descriptor, &invocation, plan)?;
...
impl ToolDispatcher for RawShellDispatcher<'_, '_> {
    fn implementation_identity(&self) -> &ImplementationIdentity {
        self.descriptor.implementation_identity()
    }
    ...
    fn start(&mut self, invocation: AuthorizedInvocation) -> Result<ToolStart, DispatchFailure> {
        let plan = self.plan.as_ref().ok_or_else(|| {
            failure::adapter(
                "shell-already-consumed",
                "raw shell dispatcher lower-layer resources were already consumed",
            )
        })?;
        validate_invocation(&self.descriptor, &invocation, plan)?;
        let creating_event = invocation.dispatch_event();
        ...
        let prepared = invocation.into_prepared();
        let plan = self.plan.take().ok_or_else(|| {
            failure::adapter("shell-already-consumed", "raw shell plan was already consumed")
        })?;
        let artifacts = self.artifacts.take().ok_or_else(|| {
            failure::adapter("shell-already-consumed", "shell artifact store was already consumed")
        })?;
...
fn bound_descriptor(
    plan: &ExecutionPlan,
) -> Result<peritus_tool_protocol::ToolDescriptor, ShellError> {
    let binding = plan
        .caller_binding()
        .ok_or_else(|| mismatch("shell execution plan has no C4 caller binding"))?;
...
fn validate_invocation(
    ...
    let binding = plan.caller_binding().ok_or_else(|| {
        failure::adapter("shell-caller-binding", "execution plan lost its exact C4 caller binding")
    })?;
