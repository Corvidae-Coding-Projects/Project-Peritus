...
H1 now has its first checked-in production controller route. A focused native run against freshly
built binaries passed the journal commit-after-durable-before-ack case by killing the staged daemon
at its real outbox checkpoint and verifying recovery from the retained journal. This is useful
evidence, not an H1 readiness claim: the remaining 42 catalog routes still need genuine component,
quota, process, and disposable-VM controls before the full report can become `Ready`.
