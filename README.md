...
facts, and refuses dirty or cross-platform inputs. On each host, eight isolated workers dynamically
consume single-probe work units from alternating catalog edges so expensive work cannot collect at
the tail. The operator reassembles results in catalog order and retains the same canonical platform
report and raw evidence. Cross-host aggregation and a separately supplied independent review remain
