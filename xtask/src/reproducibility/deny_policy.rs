...
skip = [
    ...
    { crate = "convert_case@0.10.0", reason = "Pinned Crossterm 0.29 reaches convert_case 0.10 only through derive_more's build-time macro implementation while pinned Verus macros retain convert_case 0.4; neither version crosses a Peritus API." },
    ...
    { crate = "hashbrown@0.16.1", reason = "Pinned Ratatui 0.30 reaches hashbrown 0.16 through its private Kasuari layout engine while current workspace collections use hashbrown 0.17; neither version crosses a Peritus API." },
...
mod tests {
    ...
    fn broad_or_unreasoned_exceptions_fail_closed() {
        for altered in [
            ...
            REVIEWED_POLICY.replace("convert_case@0.10.0", "convert_case"),
            REVIEWED_POLICY.replace("hashbrown@0.16.1", "hashbrown"),
